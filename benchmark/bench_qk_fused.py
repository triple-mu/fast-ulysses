...
At Wan-14B shapes (n_global=40, d=128), per world size, compares three paths that all end in the same
custom NVLink a2a (so the delta isolates the fusion):
  ...
  fused   : all_to_all_single_4d_qk                            -- 1 kernel (norm+rope fused into scatter)
Reports ms/iter and the fused-vs-unfused speedup, plus the norm+rope overhead each path adds over a2a.
"""
...
def main():
    ...
    if rank == 0:
        ...
        print(f"{'seq_global':>10} {'a2a':>9} {'unfused':>9} {'fused':>9} {'fused/unfused':>14} {'nr_unfused':>11} {'nr_fused':>10}", flush=True)
    ...
    for s_global in (20480, 46080):  # ~480p and ~720p Wan latent token counts
        ...
        if rank == 0:
            ...
            print(f"{s_global:>10} {t_a:>9.3f} {t_u:>9.3f} {t_f:>9.3f} {sp:>13.3f}x {t_u - t_a:>11.3f} {t_f - t_a:>10.3f}", flush=True)
        dist.barrier()

    if rank == 0:
        print("# ms/iter; nr_* = norm+rope overhead over plain a2a (lower is better)", flush=True)
    group.destroy()
