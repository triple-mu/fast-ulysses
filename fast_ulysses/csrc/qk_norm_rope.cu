                         ...
                         cudaStream_t     stream)
{
    if (mode == 1) {
        ...
        if (do_rope && interleaved)
            cross_head_kernel<scalar_t, true, true><<<blocks, threads, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
        else if (do_rope)
            cross_head_kernel<scalar_t, true, false><<<blocks, threads, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
        else
            cross_head_kernel<scalar_t, false, false><<<blocks, threads, 0, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
        return;
    }
    ...
    if (do_norm && do_rope && interleaved)
        per_head_kernel<scalar_t, true, true, true><<<blocks, d, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
    else if (do_norm && do_rope)
        per_head_kernel<scalar_t, true, true, false><<<blocks, d, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
    else if (do_norm)
        per_head_kernel<scalar_t, true, false, false><<<blocks, d, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
    else if (interleaved)
        per_head_kernel<scalar_t, false, true, true><<<blocks, d, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
    else
        per_head_kernel<scalar_t, false, true, false><<<blocks, d, smem, stream>>>(x, out, weight, cosb, sinb, seq, n, d, eps);
}
...
{
    if (dtype == at::kHalf)
        launch_typed<at::Half>(static_cast<const at::Half*>(x), static_cast<at::Half*>(out), weight, cosb, sinb, b, seq, n, d, do_norm, do_rope, mode, interleaved, eps, stream);
    else
        launch_typed<at::BFloat16>(static_cast<const at::BFloat16*>(x), static_cast<at::BFloat16*>(out), weight, cosb, sinb, b, seq, n, d, do_norm, do_rope, mode, interleaved, eps, stream);
...
static void check_x(const at::Tensor& x, int& b, int& seq, int& n, int& d)
{
    ...
    TORCH_CHECK(x.scalar_type() == at::kHalf || x.scalar_type() == at::kBFloat16, "x dtype must be float16 or bfloat16");
...
{
    TORCH_CHECK(t.is_cuda() && t.scalar_type() == at::kFloat && t.is_contiguous(), "weight/cos/sin must be contiguous fp32 CUDA tensors");
...
at::Tensor rms_norm(at::Tensor x, at::Tensor weight, int64_t mode, double eps)
{
    ...
    run_qk(x.data_ptr(), out.data_ptr(), fptr(weight), nullptr, nullptr, b, seq, n, d, true, false, static_cast<int>(mode), false, static_cast<float>(eps), x.scalar_type(), at::cuda::getCurrentCUDAStream());
    return out;
}
...
at::Tensor rope(at::Tensor x, at::Tensor cos, at::Tensor sin, bool interleaved)
{
    ...
    at::Tensor                out  = at::empty_like(x);
    const at::cuda::CUDAGuard guard(x.device());
    run_qk(x.data_ptr(), out.data_ptr(), nullptr, fptr(cos), fptr(sin), b, seq, n, d, false, true, 0, interleaved, 0.f, x.scalar_type(), at::cuda::getCurrentCUDAStream());
    return out;
}

at::Tensor norm_rope(at::Tensor x, at::Tensor weight, at::Tensor cos, at::Tensor sin, int64_t mode, bool interleaved, double eps)
{
    int b, seq, n, d;
    check_x(x, b, seq, n, d);
    TORCH_CHECK(mode == 0 || mode == 1, "mode must be 0 (per-head) or 1 (cross-head)");
    ...
    at::Tensor                out  = at::empty_like(x);
    const at::cuda::CUDAGuard guard(x.device());
    run_qk(x.data_ptr(), out.data_ptr(), fptr(weight), fptr(cos), fptr(sin), b, seq, n, d, true, true, static_cast<int>(mode), interleaved, static_cast<float>(eps), x.scalar_type(), at::cuda::getCurrentCUDAStream());
    return out;
}
