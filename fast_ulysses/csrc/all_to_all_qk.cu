...
struct V2<at::Half> {
    using t2 = __half2;
    static __device__ __forceinline__ float2 tof(t2 v)
    {
        return __half22float2(v);
    }
    static __device__ __forceinline__ t2 fromf(float2 f)
    {
...
template<typename scalar_t>
__global__ void
token_inv_rms_kernel(const scalar_t* __restrict__ x, float* __restrict__ inv_rms, int nd_vecs, int nd, float eps)
{
...
#pragma unroll
        for (int k = 0; k < UNROLL; ++k) {
            ...
            const int64_t src_row =
                (static_cast<int64_t>(bi) * dims.s_local + s) * dims.n_global + peer * dims.n_local + nl;
            const int64_t dst_row =
                (static_cast<int64_t>(bi) * dims.s_global + (dims.rank * dims.s_local + s)) * dims.n_local + nl;
                        ...
                        cudaStream_t         stream)
{
    if (cross_head && interleaved)  // Wan default -> plain-isomorphic fast path (eps folded in pre-pass)
        a2a_qk_chil_kernel<scalar_t, UNROLL>
            <<<blocks, threads, 0, stream>>>(x, inv_rms, peers, weight, cosb, sinb, dims, ws);
    else if (cross_head)
        a2a_qk_fused_kernel<scalar_t, UNROLL, true, false>
            <<<blocks, threads, 0, stream>>>(x, inv_rms, peers, weight, cosb, sinb, dims, ws, eps);
    else if (interleaved)
        a2a_qk_fused_kernel<scalar_t, UNROLL, false, true>
            <<<blocks, threads, 0, stream>>>(x, inv_rms, peers, weight, cosb, sinb, dims, ws, eps);
    else
        a2a_qk_fused_kernel<scalar_t, UNROLL, false, false>
            <<<blocks, threads, 0, stream>>>(x, inv_rms, peers, weight, cosb, sinb, dims, ws, eps);
}
    ...
    const void* x, float* inv_rms, const Ulysses4DDims& dims, float eps, at::ScalarType dtype, cudaStream_t stream)
{
    ...
    if (dtype == at::kHalf)
        token_inv_rms_kernel<at::Half>
            <<<blocks, 256, 0, stream>>>(static_cast<const at::Half*>(x), inv_rms, nd_vecs, nd, eps);
    else
        token_inv_rms_kernel<at::BFloat16>
            <<<blocks, 256, 0, stream>>>(static_cast<const at::BFloat16*>(x), inv_rms, nd_vecs, nd, eps);
...
{
    if (dtype == at::kHalf)
        launch_typed_qk<at::Half>(static_cast<const at::Half*>(src),
                                  inv_rms,
                                  peer_ptrs,
                                  weight,
                                  cosb,
        ...
        launch_typed_qk<at::BFloat16>(static_cast<const at::BFloat16*>(src),
                                      inv_rms,
                                      peer_ptrs,
                                      weight,
                                      cosb,
                            ...
                            const std::function<void()>& finish)
{
    ...
    if (verbose) {  // all ranks: config divergence across ranks is exactly what this diagnoses
        ...
        std::cout << "[ulysses qk-fused tune] rank=" << dims.rank << " ws=" << ws << " cross=" << cross_head
                  ...
                  << std::fixed << std::setprecision(1) << best_us << " us/iter " << std::setprecision(0) << gbps
                  << " GB/s" << std::endl;
    }
    return best;
}
